...
def mesh_waveguide(filename, wsim, hclad, hsi, wcore, hcore, gap):
    ...
    resolutions = dict(
        interface={"resolution": .01, "distance": 1},
    )
...
if __name__ == '__main__':
    omega = 9e-6
    print('lambda ', 2 * np.pi / omega / scipy.constants.c  * .1277)

    with tempfile.TemporaryDirectory() as tmpdirname:
        mesh_waveguide(wsim=10, hclad=4, hsi=1, wcore=2, hcore=.02, gap=1,
    ...
    lams, basis, xs = compute_modes(basis0, epsilon, wavelength=2 * np.pi / omega, mu_r=1, num_modes=len(conductors))
    print('lams', lams)
