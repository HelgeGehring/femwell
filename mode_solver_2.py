...
def aform(E_t, E_z, v_t, v_z, w):
    return one_over_u_r * curl(E_t) * curl(v_t) - k0 ** 2 * w['epsilon'] * dot(E_t, v_t) - one_over_u_r * dot(grad(E_z),
                                                                                                              v_t) + w[
               'epsilon'] * inner(E_t, grad(v_z)) + w['epsilon'] * E_z * v_z
...
A_ = PETSc.Mat().createAIJ(size=A.shape,
                           csr=(A.indptr, A.indices,
                                A.data))

B_ = PETSc.Mat().createAIJ(size=B.shape,
...
if __name__ == "__main__":
    ...
    plot_basis = Etbasis.with_element(ElementVector(ElementDG(ElementTriP1())))
